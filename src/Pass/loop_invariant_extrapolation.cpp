        ...
        for(int j=0;j<bb->ir_list_.size();j++)
        {
            ...
            if(res.name_==x.name_) // 如果发现了x的定值
            {
                if(x.type_==ir::Opn::Type::Array) // 如果是数组类型，一定返回false
                    return false;
                if(u.find(make_pair(loop[i],j))==u.end()) // 如果是变量类型，则看该x的定值是不是不变的，如果不是则也返回false
                {
                    return false;
                }
            }
            else if(ir->op_==ir::IR::OpKind::PARAM && ir->opn1_.type_==ir::Opn::Type::Array && ir->opn1_.name_==x.name_)
                return false;
        }
    }
    return true;
