...
Cond GenerateArmOpt::GetCondType(ir::IR::OpKind opkind, bool exchange) {
  if (exchange) {
    switch (opkind) {
...
Reg* GenerateArmOpt::NewVirtualReg() { return new Reg(virtual_reg_id++); };
...
void GenerateArmOpt::AddEpilogue(ArmBasicBlock* armbb) {
...
Reg* GenerateArmOpt::ResolveOpn2Reg(ArmBasicBlock* armbb, ir::Opn* opn) {
  if (opn->type_ == ir::Opn::Type::Imm) {  // mov rd(vreg) op2
    ...
    if (0 == opn->scope_id_) {  // 全局数组 地址量即其基址量
      ...
      } else {  // OPT: 等于-1说明是int array参数 基地址已经放在某vreg中
    ...
    if (0 == opn->scope_id_) {  // 如果是全局变量 一定有一条ldr
      ...
      Reg* vreg = NewVirtualReg();
      // FIX: var_map[opn->name_][opn->scope_id_] = vreg;
...
Operand2* GenerateArmOpt::ResolveOpn2Operand2(ArmBasicBlock* armbb, ir::Opn* opn) {
...
void GenerateArmOpt::ResolveResOpn2RdReg(ArmBasicBlock* armbb, ir::Opn* opn, CallableObjTy f) {
...
void GenerateArmOpt::AddPrologue(ArmFunction* func, ArmBasicBlock* first_bb) {
...
void GenerateArmOpt::ResetFuncData(ArmFunction* armfunc) {
  ...
  this->sp_vreg = NewVirtualReg();  // r16
            ...
            if (ir.res_.type_ == ir::Opn::Type::Array) {
              ...
              if (0 == ir.res_.scope_id_) {
                ...
                if (!symbol.IsArg()) {
                  ...
                  // this->GenImmLdrStrInst(armbb, LdrStr::OpKind::LDR, rbase, sp_vreg, symbol.stack_offset_);
                  rbase = var_map[ir.res_.name_][ir.res_.scope_id_];
                }
              }
