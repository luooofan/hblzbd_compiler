    ...
    for(auto bb : func->bb_list_){
      ...
      for(auto ir : bb->ir_list_){
        ...
        if(ir::IR::OpKind::ADD == ir->op_ ||
           ...
           ir::IR::OpKind::MUL == ir->op_ ||
           ir::IR::OpKind::DIV == ir->op_ ||
           ir::IR::OpKind::MOD == ir->op_){
          map_this_ir.clear();
          ...
          process_opn(ir, &(ir->opn1_));
          bb->use_def_chains_.push_back(map_this_ir);
        }else if(ir::IR::OpKind::JEQ == ir->op_ ||
                ir::IR::OpKind::JNE == ir->op_ ||
                ir::IR::OpKind::JLT == ir->op_ ||
                ir::IR::OpKind::JLE == ir->op_ ||
                ir::IR::OpKind::JGT == ir->op_ ||
                ir::IR::OpKind::JGE == ir->op_){
          map_this_ir.clear();
          process_opn(ir, &(ir->opn1_));
          process_opn(ir, &(ir->opn2_));
